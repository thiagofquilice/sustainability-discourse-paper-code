...
def build_troubleshooting_doc() -> str:
    return textwrap.dedent(
        ...
        ## Issues observed in the earlier workflow

        1. The older package assumed `/content` as the main read/write root.
        2. The notebook generated Python runners inline, which allowed drift between the notebook and `.py` files.
        3. `selected_micro_topics.csv` was previously produced with duplicate columns such as `share_of_non_outlier_x` and `share_of_non_outlier_y`.
        4. The older Stage 2 runner was fragile during parsing/validation and ended with `processed_rows=0`.
        5. Some inspection cells used heredocs and produced `NameError: PY`.

        ## What changes in this bundle

        - data and outputs are placed on Google Drive
        - Python runners are physical files in `runners/`
        - the notebook only calls those files
        - the `selected_micro_topics.csv` schema is corrected before export
