...
3. Binary Gemma validation using `AutoModelForCausalLM`; CPU mode is available only for small smoke tests with the same model and prompt.
4. Study-specific T2 secondary recovery materialization. A generic no-T2 mode is available for new data.
5. Source-domain BERTopic microtopic modeling across the 18 possible source-domain subgroups, with per-subgroup model artifacts saved.
...
9. Colab/Gemma annual and temporal synthesis; CPU mode is available only for small smoke tests with the same model and prompts.
